...
def main():
    ...
    if cmd in ["install", "i"]:
        ...
        if len(args) < 1:
            print_help(); return
        print(f"{YELLOW}Build (sem instalar) de {args[0]}{RESET}")
        # Aqui rodaria só até a compilação (sandbox) — instalar não
        # você pode acoplar a lógica no install.py (função build_package)

    elif cmd in ["fetch", "f"]:
        if len(args) < 1:
            print_help(); return
        print(f"{YELLOW}Baixando {args[0]}...{RESET}")
        # TODO: chamar função fetch_package()

    elif cmd in ["extract", "x"]:
        if len(args) < 1:
            print_help(); return
        print(f"{YELLOW}Extraindo {args[0]}...{RESET}")
        # TODO: chamar função extract_package()

    elif cmd in ["patch", "p"]:
        if len(args) < 1:
            print_help(); return
        print(f"{YELLOW}Aplicando patches em {args[0]}...{RESET}")
        # TODO: chamar função patch_package()

    elif cmd in ["compile", "c"]:
        if len(args) < 1:
            print_help(); return
        print(f"{YELLOW}Compilando {args[0]}...{RESET}")
        # TODO: chamar função compile_package()
